...
class RailwayExecutionError(RuntimeError):
    """Raised when an ephemeral Railway submission deployment cannot be prepared."""
...
class RailwayExecutor:
    ...
    def deploy_submission(
        ...
        *,
        ...
        if deployment_runtime_env:
            variables = [f"{key}={value}" for key, value in sorted(deployment_runtime_env.items())]
            self._run_json(
                [
                    "variable",
                    "set",
        ...
        port = str(deployment_runtime_env.get("PORT", "8000"))
        ...
        public_url = str(domain_data.get("domain") or "").strip() or _extract_public_url(
            status,
            service_name,
        )
        if not public_url:
            raise RailwayExecutionError("Unable to resolve a Railway public URL for the deployed submission.")
        ...
        *,
        ...
        for dependency in dependencies:
            if dependency.service not in ALLOWED_SERVICES:
                continue
            # Force dependency-related env vars to point at the temporary Railway services.
    ...
    def cleanup(self, result: RailwayExecutionResult) -> None:
        try:
            if result.cleanup_project:
                self._run_json(["delete", "--project", result.project_id, "--yes", "--json"], result.workspace_dir)
        finally:
    ...
    def _run_json(self, args: list[str], cwd: Path) -> dict:
        ...
        try:
            ...
            if parsed is None:
                raise RailwayExecutionError(
                    f"Expected JSON from railway {' '.join(args)}."
                )
